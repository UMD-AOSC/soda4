...
!G.Ch.		     
        if ( isnan(Grd%dat(i, j)).or. &
	     isnan(Thickness%dzt(i, j, k)).or. &
	     isnan(Dens%rho(i, j, k, taup1)).or. &
	     isnan(Grd%tmask(i, j, k)).or. &
	     isnan(Thickness%depth_zt(i, j, k)) ) then
	   write(6,*) 'From pe_tot', i,j,k, &
	     Grd%dat(i, j), &
	     Thickness%dzt(i, j, k), &
