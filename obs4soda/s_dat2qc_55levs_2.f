...
c=======================================================================
c
c  This program makes quality control on salinity data at model
c  levels for SODA
c
c=======================================================================
c
...
c
       ...
       real, dimension(idim, jdim,kdim,imn) :: clim
...
c--- open salinity climatology file
       open(31, file = '../CLIMATOLOGY/WODB/s_clim_fill_75levs.grd',
     &      access = 'direct',
     &      form = 'unformatted',
     &      status = 'old',
     &      recl = idim*jdim*4)
c
c--- read model levels climatology
       do im = 1,imn
         do k = 1,kdim
	   irec = k+(im-1)*kdim
	   read(31,rec=irec) clim(1:idim,1:jdim,k,im)
c	   write(6,*) k, clim(183:185,90,k,im)
	 enddo
       enddo
       close (31)
c
...
c--- bring counts to zero
	 ...
	 nbd3 = 0                                         ! salinity  > 43.0
	 nbd4 = 0                                         ! salinity  < 0.0
	 nbd5 = 0                                         ! departure from climatology > 5.0 (>75m, 50S-45N)
	 nbd6 = 0                                         ! departure from climatology > 2.5 (>250m, 50S-45N) 
	 nbd7 = 0                                         ! departure from climatology > 2.0 in deep ocean (>500m)
	 nbd8 = 0                                         ! vertical salinity gradient is > 0.5 (>125M)
c
c--- open salinity profiles interpolated to model levels file
         open(10, 
     &	 file = 'TMP/s_'//yst//'_55levs.dat',
c     &	 file = '../STEP_1/s_'//yst//'.dat',
     &        status = 'old')
c--- open output files for usable and bad data
c         open(20, file = '../STEP_QC/qc_s_'//yst//'.dat',
c     &        status = 'unknown')
c         open(21, file = '../STEP_QC/qc_s_'//yst//'.bad',
c     &        status = 'unknown')
c
c--- open salinity profiles interpolated to model levels file
c         open(10, file = 'TMP/s_'//yst//'.dat',
c     &        status = 'old')
c--- open output files for usable and bad data
         open(20, file = 'TMP/qc_s_'//yst//'_55levs.dat',
     &        status = 'unknown')
         open(21, file = 'TMP/qc_s_'//yst//'_55levs.bad',
     &        status = 'unknown')
     
     
c
c--- main loop to read salinity profiles at model levels
         do ipr = 1,99999999             	 
...
c
             if(prof(k).gt.43.0) then                     ! salinity is too high 
               nbd3=nbd3+1
               ibad=3
               go to 110
             endif
c
             if(prof(k).le.0.0.or.
     &          clim(ii,jj,k,month).le.0.0) then          ! salinity is too low  
               nbd4=nbd4+1
               ibad=4
               go to 110
             endif
c
             atp=abs(prof(k)-clim(ii,jj,k,month))
c
             if(jj.lt.136.and.
     &	        jj.gt.40.and.
     &          k.gt.21.and.atp.gt.7.0) then               ! obs. is too far from climatology (>75m)
               nbd5=nbd5+1
               ibad=5
               go to 110
             endif
c
             if(jj.lt.136.and.
     &          jj.gt.40.and. 
     &          k.gt.32.and.atp.gt.2.5) then              ! obs. is too far from climatology (>250m)
               nbd6=nbd6+1
               ibad=6
               go to 110
             endif
c
             if(k.gt.40.and.atp.gt.2.0) then              ! obs. is too far from climatology (>500m)
               nbd7=nbd7+1
               ibad=7
               go to 110
             endif
...
c
           do 900 k=2,kk
             ...
             grad=abs((prof(k)-prof(k-1))/(lev(k)-lev(k-1)))   ! salinity vertical gradient is too high (>125m)
             if(k.gt.26.and.grad.gt.0.5) then                         
               nbd8=nbd8+1
               ibad=8
               go to 110
             endif
...
110        continue
     ...
     &                   llv,(prof(kb), kb=1,llv)
           write(21,831) ii,jj,month,(clim(ii,jj,kb,month), kb=1,llv)
c	   write(21,*) kk, k
           ndb=ndb+1
c
50         continue
