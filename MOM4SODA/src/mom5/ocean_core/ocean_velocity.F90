...
!G.Ch.
                  if ( isnan(Velocity%accel(i,j,k,n)))  then
		    write(6,*) 'Velocity accel after advection', i,j,k,n
		    write(6,*) Velocity%accel(i,j,k,n), &
		               Velocity%advection(i,j,k,n,tau_m0), &
			       Velocity%advection(i,j,k,n,tau_m1), &
			       Velocity%advection(i,j,k,n,tau_m2)
		  endif
!end G.Ch.		  
               enddo
            enddo
...
!G.Ch.
                  if ( isnan(Velocity%accel(i,j,k,n)))  then
		    write(6,*) 'Velocity accel after press_force', i,j,k,n
		    write(6,*) Velocity%accel(i,j,k,n), &
			       Velocity%press_force(i,j,k,n)
		  endif
!end G.Ch.		  
               enddo
            enddo
...
subroutine ocean_implicit_accel(visc_cbu, visc_cbt, visc_cbu_form_drag, Time, Thickness, Adv_vel, Velocity)
  ...
  if (.not. zero_tendency .and. .not. zero_tendency_implicit) then 
      ...
      if(horz_grid == MOM_BGRID) then  
         write(stdoutunit,*) 'Start vert_friction_implicit_bgrid'
         call vert_friction_implicit_bgrid(visc_cbu, visc_cbu_form_drag, Time, Thickness, Velocity)
         write(stdoutunit,*) 'Start coriolis_force_bgrid_implicit'
...
!G.Ch.
!                  if ( isnan(Velocity%u(i,j,k,n,taup1)))  then
!		    write(6,*) 'From velocity', i,j,k,n
!		    write(6,*) Velocity%u(i,j,k,n,taup1), &
!		               Thickness%rho_dzur(i,j,k), &
!			       Velocity%u(i,j,k,n,taum1), &
!			       Velocity%accel(i,j,k,n), &
!			       Grd%umask(i,j,k)
!		  endif
!end G.Ch.		  
               enddo
...
!G.Ch.
!                  if ( isnan(Velocity%u(i,j,k,n,taup1)))  then
!		    write(6,*) 'From velocity', i,j,k,Velocity%u(i,j,k,n,taup1), &
!		                        tmp(i,j),Grd%umask(i,j,k)
!		  endif
!end G.Ch.		  
               enddo
            enddo
