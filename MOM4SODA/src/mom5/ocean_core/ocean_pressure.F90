...
!G.Ch.
                  if ( isnan(Velocity%press_force(i,j,k,1)) .or. &
		       isnan(Velocity%press_force(i,j,k,2)))  then
		    write(6,*) 'Velocity in press_force', i,j,k
		    write(6,*) Velocity%press_force(i,j,k,1), &
			       Velocity%press_force(i,j,k,2)
		    write(6,*) 'wrk1_v', wrk1_v(i,j,k,1), wrk1_v(i,j,k,2)
		    write(6,*) 'wrk2_v', wrk2_v(i,j,k,1), wrk2_v(i,j,k,2)
		    write(6,*) 'Grd umask', Grd%umask(i,j,k)
		    write(6,*) 'Thickness (dzu, geodepth_zt)', &
		               Thickness%dzu(i,j,k), &
			       Thickness%geodepth_zt(i,j,k)
		    Velocity%press_force(i,j,k,1) = 0.0
		    Velocity%press_force(i,j,k,2) = 0.0
		  endif
!end G.Ch.		  
        enddo
     enddo
