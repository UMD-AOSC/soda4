...
!! G.ch. 11.06.2021
!  <DATA NAME="dsalt_crit" UNITS="PSU" TYPE="real">
!  Critical salinity difference relative to surface for computing
!  mixed_layer_depth_dsalt . Default dsalt_crit=0.01.
!!
!  </DATA>
...
!! G.Ch. 11.06.2021
integer :: id_mixed_layer_depth_dsalt
!!
...
!! G.Ch. (11.06.2021)
!for mixed layer depth based solely on depth where SSS - salt(k) = dsalt
integer :: id_mld_dsalt =-1
!!
...
!! G.Ch. 11.06.2021
! for salinity based mixed layer 
real :: dsalt_crit=0.01 ! (PSU) critical salinity difference relative to surface 
!!
...
!! G.Ch. 11.06.2021
private mixed_layer_depth_dsalt
!! 
...
!! G.Ch. 11.06.2021
    id_mld_dsalt = register_diag_field ('ocean_model', 'mld_dsalt',       &
             Grd%tracer_axes(1:2), Time%model_time,                       &
             'mixed layer depth determined by salinity criteria ', 'm',   &
             missing_value=missing_value, range=(/0.0,1.e6/))   
!!
...
!! G.Ch. 11.06.2021    
    id_mixed_layer_depth_dsalt     = mpp_clock_id('(Ocean tracer_diag: mld_dsalt)'         ,grain=CLOCK_ROUTINE)
!!
...
!! G.Ch. 11.06.2021
  call mpp_clock_begin(id_mixed_layer_depth_dsalt)
  if(need_data(id_mld_dsalt, next_time)) then 
    call mixed_layer_depth_dsalt(Thickness,                               &
                           T_prog(index_salt)%field(isd:ied,jsd:jed,:,tau),&
                           Time)
  endif
  call mpp_clock_end(id_mixed_layer_depth_dsalt)
!!
...
!! G.Ch. 11.06.2021
!#######################################################################
! <SUBROUTINE NAME="mixed_layer_depth_dsalt">
!
! <DESCRIPTION>
!
! Calculate the depth required to reach a salinity that is 
! dsalt differ than the surface salinity. 
!
! Note:

! 1/ mixed_layer_depth_dsalt is taken with respect to the ocean surface
! at z=eta_t.
!            
! 2/ mixed_layer_depth_dsalt is no greater than the ocean depth + eta_t. 
!
! Coded January 2015 by Gennady Chepurin based on the codes of
! March 2010 by Stephen.Griffies 
!
! </DESCRIPTION>
!
subroutine mixed_layer_depth_dsalt(Thickness, salt, Time)

  type(ocean_thickness_type),   intent(in)  :: Thickness
  real, dimension(isd:,jsd:,:), intent(in)  :: salt
  type(ocean_time_type),        intent(in)  :: Time
  ...
  if (.not.module_is_initialized) then 
    call mpp_error(FATAL, &
    '==>Error from ocean_tracer_diag_mod (mixed_layer_depth_dsalt): module needs initialization ')
  endif 
     ...
     do j=jsc,jec
        do i=isc,iec
           ...
           if (kb == 0) then
               ...
               dsalt = abs(salt(i,j,1) - salt(i,j,k))
               if (dsalt >= dsalt_crit .and. wrk1_2d(i,j)==Thickness%depth_zwt(i,j,kb)) then
                   wrk1_2d(i,j) = Thickness%depth_zt(i,j,km1)  &
                                + (dsalt_crit/(dsalt+epsln)) &
                                 *(Thickness%depth_zt(i,j,k) - Thickness%depth_zt(i,j,km1))
               endif
               wrk1_2d(i,j) = wrk1_2d(i,j) * Grd%tmask(i,j,1)
           endif
  ...
  call diagnose_2d(Time, Grd, id_mld_dsalt, wrk1_2d(:,:))


end subroutine mixed_layer_depth_dsalt
! </SUBROUTINE>  NAME="mixed_layer_depth_dsalt"
!!
