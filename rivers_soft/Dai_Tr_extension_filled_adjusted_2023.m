...
%
ind = find(flow_new<0.0);  % filter negative discharge data 
flow_new(ind) = NaN;       % filter negative discharge data
%
% Compute monthly climatology for each river.  We'll use this to fill in missing values
%
clim = zeros(12,size(flow_new,1));   % initialize the climatology [12x925]
for i = 1:size(flow_new,1);   % loop over rivers and reshape each so that there are 12 columns for the 12  months
	far = reshape(flow_new(i,:),12,[]);
	faa = mean(far,2,'omitnan');   %compute the climotological monthly average ignoring NANs
	clim(:,i) = faa;   %load it into fa_clim
end
yrs = size(far,2);
clear far faa
% fill gaps in the data with climatology
flow_fll = flow_new;
for i = 1:size(flow_new,1);   % loop over rivers 
    flow_clim = repmat(clim(:,i),yrs,1);
    ind = find(isnan(flow_new(i,:)) == 1 | flow_new(i,:) < 1.0 );   % find cell indexes where new data exist
    flow_fll(i,ind) = flow_clim(ind); 
end
%
...
flow3 = flow_fll;
nn = 3;
ind = indatl(find(indatl>nn)); flow3(ind,:) = flow_fll(ind,:)*1.2793;  % adjust Atlantic ocean rivers
ind = indarc(find(indarc>nn)); flow3(ind,:) = flow_fll(ind,:)*1.3479;  % adjust Arctic ocean rivers
ind = indind(find(indind>nn)); flow3(ind,:) = flow_fll(ind,:)*1.6946;  % adjust Indian ocean rivers
ind = indpac(find(indpac>nn)); flow3(ind,:) = flow_fll(ind,:)*2.2305;  % adjust Pacific ocean rivers
%
% write extended and adjusted discharge data in NetCDF file
%fname = rdir+'Dai_Tr_extension_2023.nc';        % data extended to 12/2023
%fname = rdir+'Dai_Tr_extension_filled_2023.nc'; % data extended to 12/2023 and filled with climatology
fname = rdir+'Dai_Tr_extension_filled_adjusted_2023.nc'; % data extended to 12/2023, filled with climatology, and adjusted for each ocean basin
time(1429:1488)=(floor(([1429:1488]-0.5)/12)+1900)*100 +[1429:1488]-floor(([1429:1488]-0.5)/12)*12;  % extend time to 12/2023
...
%ncwrite(fname,'FLOW',flow_new);   % data extended to 12/2023
%ncwrite(fname,'FLOW',flow_fll);   % data extended to 12/2023 and filled with climatology
ncwrite(fname,'FLOW',flow3);   % data extended to 12/2023, filled with climatology, and adjusted for ocean each basin
